...
public class WalkingRobotSimulation_874 {
    ...
    public int robotSim(int[] commands, int[][] obstacles) {
        HashSet<Pair<Integer,Integer>> obs = new HashSet<>();
        for(int[] o : obstacles) obs.add(new Pair<>(o[0], o[1]));
        int[] cur = new int[2];
        //北,东,南,西
        int[][] dir = {{0,1},{1,0},{0,-1},{-1,0}};
        int dir_cur = 0;
        for(int c : commands) {
            if(c == -1) dir_cur = (4 + dir_cur + 1) % 4;
            else if(c == -2) dir_cur = (4 + dir_cur - 1) % 4;
            else {
                for(int i = 0; i < c; i++) {
                    int x = cur[0] + dir[dir_cur][0];
                    int y = cur[1] + dir[dir_cur][1];
